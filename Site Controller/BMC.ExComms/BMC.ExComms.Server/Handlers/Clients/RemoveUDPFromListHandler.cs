...
namespace BMC.ExComms.Server.Handlers.Client
{
    [FFTgtHandler_External_GMU(
        ...
        Request = new Type[] { typeof(FFTgt_H2G_Client_RemoveUDPFromList) }
        )]
    internal sealed class FFTgtHandler_Client_RemoveUDPFromList_H2G_GMU
        : FFTgtHandler
    {
        protected override bool OnProcessMessageH2GExternal_GMU(FFTgtExecutionContext context, IFreeformEntity_MsgTgt target)
        {
            FFTgt_H2G_Client_RemoveUDPFromList tgtSrc = target as FFTgt_H2G_Client_RemoveUDPFromList;
            return ExCommsExecutorFactory.RemoveInstallation(tgtSrc.InstallationNo);
        }
    }
}
