...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rsp_getAnalysisPerformanceData_Period_Read]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[rsp_getAnalysisPerformanceData_Period_Read]
GO
...
--- Outputs:    dataset
----Execute:	exec rsp_getAnalysisPerformanceData_Period_Read 'site',0,0,0,'80,79,78','manufac',1,'',48
--- ======================================================================================================================
--- 
--- Revision History
--- 
...
---------------------------------------------------------------------------------- 
CREATE PROCEDURE [dbo].[rsp_getAnalysisPerformanceData_Period_Read]
...
Select VW_Read.Read_No as Read_Count,Period_Id AS Calendar_Period_Ref,
(VTP) as VTP,(Hold) as Hold,(RDCCashIn) as RDCCashIN,
(RDCCashOut) as RDCCashOut,(TicketsIN) as TicketsIN,(HandPay) as HandPay,
Machine_class_id			= case when (@PerformanceDataType	= 'machine'			or @Groupby='barpos')									  THEN Machine_class.Machine_Class_ID else ''  end, 
Manufacturer_ID				= case when @PerformanceDataType	= 'manufac'																	  THEN Manufacturer.Manufacturer_ID else ''  end,
...
--This is the customised where clause. Great logic, thanks Carl :-)
WHERE VW_Read.Period_ID IN (select * from #temptableWeekID)
 AND (
				(
...
SELECT Calendar_Period_Ref, 
count(distinct Read_Count) as ReadCount,
SUM(VTP) as VTP,AVG(Hold) as Hold,
...
isnull(SUM(HandPay),0) as HandPay,
...
Manufacturer_ID				= case when  @PerformanceDataType	= 'manufac'																	  THEN Manufacturer_ID else ''  end,
...
ORDER BY CASE WHEN (@PerformanceDataType = 'general:subcomp' or @Groupby='subcomp') THEN 
			  ...
			  WHEN @PerformanceDataType = 'machine' THEN 
			  ...
			  WHEN @PerformanceDataType = 'machinetype' THEN 
