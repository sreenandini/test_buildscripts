...
IF EXISTS (
       SELECT *
       FROM   sys.objects
       WHERE  OBJECT_ID = OBJECT_ID(N'[dbo].[usp_AddOrCopyTermsProfilesForMachineTypes]')
              AND TYPE IN (N'P', N'PC')
   )
    DROP PROCEDURE [dbo].[usp_AddOrCopyTermsProfilesForMachineTypes]
GO

CREATE PROCEDURE [dbo].[usp_AddOrCopyTermsProfilesForMachineTypes] 
(
    @MachineTypeID   INT,
    @TermsGroupID    INT,
    @TermsProfileID  INT,
...
BEGIN
	...
	BEGIN
	    ...
	    SELECT @TermsGroupID,
	           ...
	           Terms_Profile_GPT_Deferred_Remittance
	    FROM   Terms_Profile
	    WHERE  Terms_Profile_ID = @TermsProfileID
	END
	ELSE
	BEGIN
	    INSERT INTO dbo.Terms_Profile
