...
IF EXISTS (
       ...
       WHERE  SPECIFIC_SCHEMA = N'dbo'
              AND SPECIFIC_NAME = N'rsp_ecGetDropBatchBreakDownByWeek'
   )
    DROP PROCEDURE dbo.rsp_ecGetDropBatchBreakDownByWeek
GO

CREATE PROCEDURE dbo.rsp_ecGetDropBatchBreakDownByWeek
	@Site_ID  INT = 0,
	@Week_ID INT = 0
AS
	/*****************************************************************************************************
...
BEGIN
	...
	SELECT CD.IsSAS,
	       ...
	       --DecWinOrLoss
	       (
	           cd.Declared_Notes + DecTicketBalance - DecHandpay + Net_Coin + (
	               CASE 
	                    WHEN @IsAFTIncludedInCalculation = 1 THEN (CD.DecEftIn / 100.00)
	                         -(CD.DecEftOut / 100.00)
	                    ELSE 0
	       ...
	       CD.Declared_Notes,
	       CD.Note_Var,
	       CD.DecTicketBalance + (CASE WHEN ISNULL(@sAddShortpay,'True') = 'True' THEN 0 ELSE Shortpay END) DecTicketBalance,
	       CD.Shortpay,
	       CD.Void,
	       (CD.DecTicketBalance - CD.Ticket_Var) AS RDCTickets,
	       CD.Ticket_Var + (CASE WHEN ISNULL(@sAddShortpay,'True') = 'True' THEN 0 ELSE Shortpay END) Ticket_Var,
	       CD.DecHandpay,
	       CD.RDCHandpay,
	       CD.Handpay_Var,
