...
public partial class DetailControl : System.Web.UI.UserControl
{
    ...
    public DetailControl()
    {
        ...
        try
        {
            if (int.TryParse(ConfigurationManager.AppSettings["InActiveServerInterval"], out configValue))
            {
                _InActiveServerInterval = configValue < 10 ? 10 : configValue;
            }
        }
        catch (Exception)
        {
    ...
    public void PopulateSite(string SiteName)
    {
        ...
        bool isTerminated = (Session["isTerminated"] != null && (bool)Session["isTerminated"]);
        if (isTerminated)
        {
            GeneralInformation.InnerText = "            Site Terminated, Details Not Available";
            div_Info.Visible = false;
        }
        else if (string.IsNullOrEmpty(siteStatus))
        {
            GeneralInformation.InnerText = "          Site Status not found, Please Check Exchange Server Communication";
            div_Info.Visible = false;
        }
        else
    ...
    public void PoplutateSiteDetails(string siteName, string siteStatus, bool IsHistory)
    {
        ...
        if (!string.IsNullOrEmpty(siteStatus))
        {
            ...
            if (!_ForHistory)
            {
                SiteStatusDetails _siteStatus = GuardianBL.GetSiteStatusDetails(lastProcessedDetails, Convert.ToString(Session["UserName"]), Convert.ToInt32(ConfigManager.Read("ProxyTimeOut")),0);
            ...
            if(!_ForHistory)
            {
                ...
                if (Convert.ToInt64(ts_subtract.TotalMinutes) > Convert.ToInt64(ts_interval.TotalMinutes))
                {
                    GeneralInformation.InnerText = "          Site Status not found, Please Check Exchange Server Communication";
                    div_Info.Visible = false;
                    return;
            ...
            if (string.IsNullOrEmpty(lastProcessedDetails.DateTime))
            {
                ...
                timeSpanDiff = SetLastUpdate(siteName, lastProcessedDetails.DateTime);
            PopulateOtherDetails(siteName, lastProcessedDetails);
    ...
    private void ClearControls()
    {
        ...
        lt_GMUMachine.Text = string.Empty;
        lt_GMUServer.Text = string.Empty;
    ...
    protected void GridViewSiteInfo_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        try
        {
            ...
            if(IPAddress=="''")
            {
                GridViewSiteInfo.Rows[rowindex].Cells[6].Text = "Reboot Failed";
                return;
            }
            if (result.ToUpper() == "TRUE")
            {
                ...
                AuditReboot("Reboot Successful for Asset No:" + Asset + "and IPAddress:" + IPAddress);
            }
            ...
            else
            {
                ...
                AuditReboot("Reboot Failed for Asset No:" + Asset + "and IPAddress:" + IPAddress);
            }
        }
        catch (Exception ex)
        {
            ...
            GridViewSiteInfo.Rows[rowindex].Cells[6].Text = "Reboot Failed for IP:" + IPAddress;
            AuditReboot("Reboot Failed for Asset No:" + Asset);
        }
        finally
        {
            if (boot != null)
    ...
    #endregion Reboot
    #region Audit Reboot
    private void AuditReboot(string sDesc)
    {
        try
        {
            objAudit = new AuditViewerBusiness(GuardianDBHelper.EnterpriseConnectionString);
            int iUserId = (Session["UserID"] != null) ? Convert.ToInt32("0" + Session["UserID"]) : 0;
            string sUserName = (Session["DisplayName"] != null) ? Session["DisplayName"].ToString() : string.Empty;
            objAudit.InsertAuditData(new Audit.Transport.Audit_History
            {
                Audit_User_ID = iUserId,
                Audit_User_Name = sUserName,
                AuditModuleName = ModuleName.RebootGMU,

                Audit_Screen_Name = "Guardian Detail Control Screen",
                Audit_Desc = sDesc,
