...
namespace BMC.MeterAdjustmentTool.Helpers
{
    ...
    internal static class TripleDESEncryption
    {
        ...
        /// <param name="key">The key.</param>
        /// <returns>Encryped string</returns>
        public static byte[] EncryptToBytes(string original)
        {
            try
            {
                byte[] buff = Encoding.Default.GetBytes(original);
                byte[] kb = Encoding.Default.GetBytes(EncryptionConstants.SYMMETRIC_KEY);
                return Encrypt(buff, kb);
            }
            catch { return null; }
        }
        ...
        /// <param name="encoding">The encoding.</param>
        /// <returns>Decryped string</returns>
        public static string DecryptFromBytes(byte[] encrypted, string key, Encoding encoding)
        {
            try
            {
                byte[] buff = encrypted;
        ...
        /// <param name="encoding">The encoding.</param>
        /// <returns>Decryped string</returns>
        public static string DecryptFromBytes(byte[] encrypted)
        {
            return DecryptFromBytes(encrypted, EncryptionConstants.SYMMETRIC_KEY, Encoding.Default);
        }
    ...
    {
        internal const string SYMMETRIC_KEY = "AnCaGaKaMaMaNaPoRaReSuVi";
        internal const string BGS_ENCRYPTIONKEY = "B411yT3cH";
    }
}
